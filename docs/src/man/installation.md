...
[SimpleGraphs](https://github.com/singularitti/SimpleGraphs.jl).
...
version of SimpleGraphs, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/SimpleGraphs.jl"
```
...
our [GitHub repository](https://github.com/singularitti/SimpleGraphs.jl)
for new releases.
Once we release a new version, you can update SimpleGraphs by typing

```@repl
using Pkg
Pkg.update("SimpleGraphs")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SimpleGraphs")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall SimpleGraphs.

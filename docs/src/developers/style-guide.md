...
!!! info
    The style guide is always a work in progress, and not all SimpleGraphs code
    follows the rules. When modifying SimpleGraphs, please fix the style violations
...
SimpleGraphs uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/SimpleGraphs.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the SimpleGraphs directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to SimpleGraphs have
